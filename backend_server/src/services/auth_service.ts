...
export class AuthService {
  ...
  public static async registerStudent(input: RegisterStudentInput) {
    ...
    // Registration Number: strictly 7 to 8 digits (e.g. 2080033)
    if (!/^\d{7,8}$/.test(cleanRegNo)) {
      throw new Error('Registration Number must be 7 or 8 digits (e.g. 2080033).');
    }

    // University Roll Number: formatted as 90/COURSE_NAME/6-DIGIT_NUMBER (e.g. 90/MCA/250028)
    if (!/^\d{2}\/[A-Za-z0-9_.\-\s]+\/\d{6}$/i.test(cleanUniRoll)) {
      throw new Error("University Roll Number must match format '90/COURSE_NAME/6-DIGIT_NUMBER' (e.g. 90/MCA/250028).");
    }
  ...
  public static async registerTeacher(input: RegisterTeacherInput) {
    const cleanEmail = input.email.toLowerCase().trim();
    // ALL faculty registrations through the portal/app MUST be approved by the Administrator
    const role = 'TEACHER';
    ...
    if (existingUser) {
      throw new Error('Email is already registered. If pending approval, please await administrator verification before signing in.');
    }
